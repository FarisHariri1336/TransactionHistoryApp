    ...
    <View style={{ height: sh72, ...rowCenterVertical, ...px(sw24), ...spaceBetweenHorizontal, ...customStyle }}>
      {leftIcon ? (
        <Pressable onPress={leftIcon.onPress}>
          <Icon color={leftIcon.color || colorGreen._1} name={leftIcon.name || "arrow-left"} size={leftIcon.size || sw24} />
        </Pressable>
      ) : (
      ...
      {rightIcon ? (
        <Pressable onPress={rightIcon.onPress}>
          <Icon color={rightIcon.color || colorGreen._1} name={rightIcon.name || "close"} size={rightIcon.size || sw24} />
        </Pressable>
      ) : (
        <View style={{ width: sw24 }} />
      )}
